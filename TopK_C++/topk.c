...
static inline char *topKStrndup(const char *s, size_t n) {
    char *ret = (char*)TOPK_CALLOC(n + 1, sizeof(char));
    if (ret)
        memcpy(ret, s, n);
    return ret;
}
...
TopK *TopK_Create(uint32_t k, uint32_t width, uint32_t depth, double decay) {
    ...
    topk->depth = depth;
    topk->decay = decay;
    topk->data = (Bucket*)TOPK_CALLOC(((size_t)width) * depth, sizeof(Bucket));
    topk->heap = (HeapBucket*)TOPK_CALLOC(k, sizeof(HeapBucket));
...
HeapBucket *TopK_List(TopK *topk) {
    HeapBucket *heapList = (HeapBucket*)TOPK_CALLOC(topk->k, (sizeof(*heapList)));
